...
allNicknames = [
    "Delightful Danny",
    "Y'all are the best coworkers",
    "Wholesome Harry",
    "Who wants a hug guys",
    "Accepting Andy",
    "Can I get a high five",
    "Loving Coworker",
    "Wholesomeness is my middle name",
    "Captain Friendly",
]
allChuckJokes = [
...
def compliment():
    return allCompliments[random.randint(0, 99)]


def chuckNorris():
    return allChuckJokes[random.randint(0, 573)]
...
async def on_message(message):
    ...
    if ":reactron" in message.content.lower():
        ...
        try:
            ...
            if "poll" in message2.content.lower():
                await message.add_reaction(emojiList["yes"])
                await message.add_reaction(emojiList["no"])
            else:
                for item in content:
    ...
    if ":help" in message.content.lower():
        ...
        while 0 < len(emoList):
            ...
            if len(emoList) == 4:
                await message.author.send(
                    emoList.pop() + emoList.pop() + emoList.pop() + emoList.pop()
                )
            if len(emoList) == 3:
                await message.author.send(emoList.pop() + emoList.pop() + emoList.pop())
            if len(emoList) == 2:
                await message.author.send(emoList.pop() + emoList.pop())
            if len(emoList) == 1:
                await message.author.send(emoList.pop())
        await message.author.send(
            "\nTo use reactron all you have to do is count the number of messages between the message you want to react on and the most recent message. Then you will use the command :reacton with a hyphen separated list of reactions and the number of messages between the message you want to react on and the current message. So if you want to react with the yes and no reactions on the 5th message up you would use :reactron -yes-no 5. \n\n**:reactron reaction_name #**\n\nReactron can also work across channels as well if it is in them. The list of reactions and their keywords can be found above and the following is a list of special commands.\n\n"
            + "**:poll #** - Turns the comment where :poll is used into a poll with reactions.\n"
            + "**:dadjoke** - Tells a random dad joke.\n"
            + "**:repost** - Posts a repost image.\n"
            + "**:mayberepost** - Posts a maybe repost image.\n"
            + "**:compliment** - Compliments the user. Additionally using @username or multiple will compliment each referenced user.\n"
            + "**:randomcompliment** - Compliments a random user.\n"
            + "**:flip** - Flips a table.\n"
            + "**:2flips** - Flips 2 tables.\n"
            + "**:alltheflips** - Flips ALL the tables (50).\n"
            + "**:unflip** - Unflips a table.\n"
            + "**:2unflips** - Unflips 2 tables.\n"
            + "**:alltheunflips** - Unflips All the tables (50).\n"
            + "**chuck norris** - Using chuck norris in a sentence posts a random Chuck Norris Fact.\n"
        )
    if ":dadjoke" in message.content.lower():
        try:
            ...
            print("Failed")
    if ":repost" in message.content.lower():
        await message.channel.send(
            "https://cdn.discordapp.com/attachments/476837202886918145/524680451944742922/general.png"
        )
    if ":mayberepost" in message.content.lower():
        await message.channel.send(
            "https://cdn.discordapp.com/attachments/476837202886918145/562665008253632533/2xkhn3.png"
        )
    if ":compliment" in message.content.lower():
        if not message.mentions:
            await message.channel.send(message.author.mention + " " + compliment())
        else:
            for i in message.mentions:
                await message.channel.send(i.mention + " " + compliment())
    if "chuck norris" in message.content.lower():
        if not message.author.bot:
            await message.channel.send(
                "Did someone say Chuck Norris? Did you know: " + chuckNorris()
            )
    if ":randomcompliment" in message.content.lower():
        await message.channel.send(
            random.choice(message.channel.members).mention + " " + compliment()
        )
    if ":flip" in message.content.lower():
        await message.channel.send("(╯°□°）╯︵ ┻━┻")
    if ":2flips" in message.content.lower():
        await message.channel.send("┻━┻ ︵╰ (°□°）╯︵ ┻━┻")
    if ":alltheflips" in message.content.lower():
        await message.channel.send(
            "`┻━┻ ︵ ╰ (°□°）╯ ︵ ┻━┻\n┻━┻ ︵︵┻━┻  "
            + "┻━┻︵︵ ┻━┻\n┻━┻ ︵︵┻━┻  ┻━┻︵︵ ┻━┻\n┻━┻ ︵︵┻━┻  ┻━┻︵︵ ┻━┻\n"
            + "┻━┻ ︵︵┻━┻  ┻━┻︵︵ ┻━┻\n┻━┻ ︵︵┻━┻  ┻━┻︵︵ ┻━┻\n┻━┻ ︵︵┻━┻  "
    ...
    if ":unflip" in message.content.lower():
        await message.channel.send("┬─┬ノ( º _ ºノ)")
    if ":2unflips" in message.content.lower():
        await message.channel.send("┬─┬ノ(º _ º)レ┬─┬")
    if ":alltheunflips" in message.content.lower():
        await message.channel.send(
            "`┬─┬ノ(º _ º)レ┬─┬\n┬─┬ ┬─┬  "
            + "┬─┬ ┬─┬\n┬─┬ ┬─┬  ┬─┬ ┬─┬\n┬─┬ ┬─┬  ┬─┬ ┬─┬\n┬─┬ ┬─┬  "
            + "┬─┬ ┬─┬\n┬─┬ ┬─┬  ┬─┬ ┬─┬\n┬─┬ ┬─┬  ┬─┬ ┬─┬\n┬─┬ ┬─┬  "
